...
from meta.data.CorrectCifarDataset import NoisyLabelsCifarStaticDataset, CifarBatchSampler
...
class CifarNoisyLabelsExperiment(CifarExperiment):
    ...
    def run(self):
        ...
        CifarTrainingDataset = NoisyLabelsCifarStaticDataset(
                self.args.root_dir, 
                'train', 
        ...
        CifarValidationDataset = NoisyLabelsCifarStaticDataset(
                self.args.root_dir, 
                'val', 
		self.args.noise_percent,
    ...
    def evaluate(self):
        ...
        CifarTestDataset = NoisyLabelsCifarStaticDataset(
                self.args.root_dir, 
                'test', 
		self.args.noise_percent,
    ...
    def evaluate_easy(self):
        ...
        see.logs.cache['test_loss_easy'] = self.train_op.mean_outer_loss(TestDataloader)
        test_accuracy = self.train_op.get_accuracy(TestDataloader)
        see.logs.cache['test_accuracy_easy'] = test_accuracy
        print('test_accuracy_easy={}'.format(test_accuracy))
    ...
    def evaluate_hard(self):
        ...
        see.logs.cache['test_loss_hard'] = self.train_op.mean_outer_loss(TestDataloader)
        test_accuracy = self.train_op.get_accuracy(TestDataloader)
        see.logs.cache['test_accuracy_hard'] = test_accuracy
        print('test_accuracy_hard={}'.format(test_accuracy))
...
if __name__ == '__main__':
    ...
    for run in range(runs):
        ...
        updated_args = CifarNoisyLabelsExperiment.update_args_from_config(args, config)
        ...
        exp = CifarNoisyLabelsExperiment(args, config, train_op)
